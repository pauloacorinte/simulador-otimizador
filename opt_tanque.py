...
def Otimizador_Tanque(E_bat_max, bateria_limite, M_pulv_max, M_bat, M_vazio, COAXIAL_80, g, 
    ...
    while energia_voo <= E_bat_max * (1 - bateria_limite) and M_Pulv <= M_pulv_max:
        
        MTOW = M_bat + M_vazio + M_Pulv
        Massinha = np.linspace(MTOW - M_Pulv, MTOW, 1000)
        
        # Cálculo da potência W
        W = COAXIAL_80 * Massinha / ((1 / g / (((1 / (eta_escmotor * eta_helice)) * 
        ...
        # Energia para ida
        energia_ida = (COAXIAL_80 * MTOW  / ((1 / g / (((1 / (eta_escmotor * eta_helice)) * 
            (np.sqrt(MTOW * g / (2 * rho * A * n_motores))))))) * 
        ...
        energia_subida = ((MTOW * g * zi) / 3600) + (zi / v_subida) * \
                         (COAXIAL_80 * MTOW * 1000 / ((1000 / g / (((1 / (eta_escmotor * eta_helice)) * 
                         (np.sqrt(MTOW * g / (2 * rho * A * n_motores))))))) / 3600)
        ...
        energia_curvas = ((MTOW * (v_pulv**2) / 2) / 3600) * delta_x / faixa
        
        # energia_volta = (COAXIAL_80*(M_bat+M_vazio)*1000/((1000/9.80665/(((1/(eta_escmotor*eta_helice)))*(np.sqrt((M_bat+M_vazio)*9.80665/(2*rho*A*n_motores)))))))*((((x_rtl+delta_x)**2+(y_rtl+delta_y)**2)**0.5)/v_desloc)/3600
