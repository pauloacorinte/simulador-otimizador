...
celulas = 14
# cap_bat = 30000*0.751   
cap_bat = np.linspace(21000, 21215, 50)                                                                                                      
M_vazio = 38  
M_bat = 12.9
...
for nn in range(len(cap_bat)):
    print("AAA")    
    M_tot_in = M_pulv_max + M_vazio + M_bat           
    E_bat_max = (cap_bat[nn]/1000)*3.7*celulas           #[Wh]
    print(E_bat_max)
    P_sensores = 0;
    P_LED = 100
    P_sist = 38.71
    P_bombas = 95.04
    dt = 0.1                  #[s]
    t_prep = 300; t_abs_calda = M_pulv_max*60/50+8; t_abs_comb = 40; t_desloc_pre_op = 2520  
            ...
            if (OP[i] == "RTL FIM"):
                ...
                if (x[i+1] == 0 and y[i+1] == 0 and z[i+1] == 0):
                    ...
                    if OTIMIZAR_TANQUE == "SIM":
                        ...
                        energia_tanque = 0
                        energia_bateria = (cap_bat[nn]/1000)*3.7*celulas
                        energia_ida = 0
                        energia_voo = 0
                        energia_volta = 0
                        M_Pulv=0
    ...
    capacidade_vector.append(cap_bat[nn])
    ...
    # (preco_por_ha_real[tanque],preco_drone_bateria[tanque]) = custos(M_pulv_max,cap_bat[nn],area_total,t_de_voo[i]/3600,voo,math.ceil(area_total/(X**2/10**4)),X**2/10**4)
    ...
    Dist_voo_corrigida.append(Dist_voo[0])
    ...
    for n in range(len(X_rtl_por_voo)):
        ...
        ErroRTW.append(100*(abs(dist_simulado-dist_ensaio)/dist_ensaio))

    ErroDistVooRel = []
    ErroDistVooAbs = []
    print(len(Dist_voo_corrigida))
    if len(Dist_voo_corrigida) == len(Dist_ensaio_voo):
        ...
        capacidade.append(cap_bat[nn])
        Desvio_rel.append(desvio_rel_0)
